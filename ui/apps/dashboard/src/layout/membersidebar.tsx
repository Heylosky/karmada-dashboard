...
const MemberSidebar: FC<SidebarProps> = ({ collapsed }) => {
  ...
  const filteredMenuItems = useMemo(() => {
    ...
    const menuInfo = traverseMemberMenuConfig(data.member_menu_configs);
...
function traverseMemberMenuConfig(member_menu_configs: menuConfig[],): Record<string, boolean> {
  let menuInfo = {} as Record<string, boolean>;
  for (const menu_config of member_menu_configs) {
    menuInfo[menu_config.sidebar_key] = menu_config.enable;
    const childrenMenuInfo = menu_config.children
      ? traverseMemberMenuConfig(menu_config.children)
      : {};
    menuInfo = {
      ...menuInfo,
      ...childrenMenuInfo,
