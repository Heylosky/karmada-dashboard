...
const MemberNamespacePage = () => {
  const [searchFilter, setSearchFilter] = useState('');
  const { cluster } = useCluster();
  const { data, isLoading, refetch } = useQuery({
    queryKey: ['GetMemberNamespaces', searchFilter],
    queryFn: async () => {
      const query: DataSelectQuery = {};
      if (searchFilter) {
        query.filterBy = ['name', searchFilter];
      }
      const clusters = await GetMemberNamespaces({clustername: cluster}, query);
...
export default MemberNamespacePage;
