...
import { WorkloadEvent } from '@/services/workload.ts';
import {
  GetMemberWorkloadDetail,
  GetMemberWorkloadEvents,
} from '@/services/member/workload';
...
const WorkloadDetailDrawer: FC<WorkloadDetailDrawerProps> = (props) => {
  ...
  const { data: detailData, isLoading: isDetailDataLoading } = useQuery({
    queryKey: ['GetMemberWorkloadDetail', cluster, kind, name, namespace],
    queryFn: async () => {
      const workloadDetailRet = await GetMemberWorkloadDetail({
        clustername: cluster,
        namespace,
        name,
        kind,
  ...
  const { data: eventsData } = useQuery({
    queryKey: ['GetMemberWorkloadEvents', kind, name, namespace],
    queryFn: async () => {
      const workloadEventsRet = await GetMemberWorkloadEvents({
        clustername: cluster,
        namespace,
        name,
        kind,
    ...
    >
      <Card title={i18nInstance.t('9e5ffa068ed435ced73dc9bf5dd8e09c')} bordered>
        ...
        <div className="mb-4">
          ...
          <div>
            {detailData?.objectMeta?.labels && Object.entries(detailData.objectMeta.labels).map(([key, value]) => (
              <Tag key={key}>
              {key}: {value}
              </Tag>
            ))}
        ...
        <div>
          ...
          <div>
            ...
            {detailData?.objectMeta?.annotations && Object.entries(detailData.objectMeta.annotations).map(([key, value]) => (
              <Tag key={key}>
              {key}: {value}
              </Tag>
            ))}
